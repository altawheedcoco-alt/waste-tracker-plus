...
const eps = ["ep19-light", "ep20-dark", "ep20-light"];
...
const browser = await openBrowser("chrome", { browserExecutable: "/bin/chromium", chromiumOptions: { args: ["--no-sandbox", "--disable-gpu", "--disable-dev-shm-usage"] }, chromeMode: "headless-shell" });
for (const ep of eps) {
  const t0 = Date.now();
  console.log(`🎬 ${ep}...`);
