...
const DisposalOperations = () => {
  ...
  const { data: operations, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      let query = supabase
        ...
        .select(`
          *,
          disposal_facility:disposal_facilities(name),
      ...
      if (methodFilter !== 'all') {
        query = query.eq('disposal_method', methodFilter);
      }
  ...
  const filteredOperations = operations?.filter((op: any) => {
    ...
    return (
      op.waste_description?.toLowerCase().includes(query) ||
      op.waste_type?.toLowerCase().includes(query) ||
      op.manifest_number?.toLowerCase().includes(query) ||
      op.certificate_number?.toLowerCase().includes(query)
    );
  });
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'completed':
        return <Badge className="bg-green-500/10 text-green-600 hover:bg-green-500/20 gap-1"><CheckCircle className="w-3 h-3" /> مكتمل</Badge>;
      case 'processing':
        return <Badge className="bg-amber-500/10 text-amber-600 hover:bg-amber-500/20 gap-1"><Clock className="w-3 h-3" /> قيد المعالجة</Badge>;
      case 'pending':
        return <Badge className="bg-blue-500/10 text-blue-600 hover:bg-blue-500/20 gap-1"><Clock className="w-3 h-3" /> قيد الانتظار</Badge>;
      case 'rejected':
            ...
            <CardContent className="pt-6">
              <div className="flex flex-col md:flex-row gap-4">
                <div className="relative flex-1">
                  ...
                  <Input
                    placeholder="بحث بالوصف، رقم البيان، أو رقم الشهادة..."
                    value={searchQuery}
                ...
                <Select value={methodFilter} onValueChange={setMethodFilter}>
                  <SelectTrigger className="w-full md:w-[180px]">
                    <SelectValue placeholder="طريقة التخلص" />
                  </SelectTrigger>
                  <SelectContent>
