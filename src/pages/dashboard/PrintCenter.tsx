...
<div class="footer">
  ...
  <p>هذا التقرير آلي ولا يحتاج إلى توقيع</p>
      ...
      console.error('Print center error:', error);
    ...
    <div className="space-y-6" dir="rtl">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground text-sm">طباعة المستندات المجمعة - إقرارات، شهادات، مانيفست، فواتير، إيصالات</p>
        </div>
      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
              {DOC_TYPES.filter(d => d.id !== 'all').map(dt => (
                <div key={dt.id} className="flex items-center gap-3 p-4 rounded-xl border bg-card hover:shadow-sm transition-shadow">
                  <div className="w-10 h-10 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                    {dt.icon}
                  </div>
                  <div>
                    <p className="font-medium text-sm">{dt.label}</p>
                    <p className="text-xs text-muted-foreground">متاح للطباعة</p>
                  </div>
                </div>
              ))}
            </div>
            <div className="mt-6 p-4 rounded-xl bg-muted/50 border">
              <p className="text-sm text-muted-foreground">
                💡 اختر الفلاتر المناسبة من القائمة الجانبية ثم اضغط "طباعة التقرير المجمع" لاستخراج جميع المستندات المحددة في تقرير واحد جاهز للطباعة.
              </p>
            </div>
          </CardContent>
        </Card>
