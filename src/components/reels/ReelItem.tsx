...
import { Heart, MessageCircle, Share2, Bookmark, Play, Volume2, VolumeX, MoreHorizontal, Trash2, Eye, Music, UserPlus } from 'lucide-react';
...
const ReelItem = memo(({ reel, isActive, onLike, onComment, onBookmark, onShare, onDelete }: ReelItemProps) => {
  ...
  return (
    <div className="relative w-full h-full bg-black snap-start snap-always flex-shrink-0 overflow-hidden">
      ...
      <div className="absolute bottom-5 left-4 right-[72px] z-20">
        ...
        <div className="flex items-center gap-2.5 mb-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            {reel.org_name && (
              <p className="text-white/50 text-[11px] truncate">{reel.org_name}</p>
            )}
          </div>
        </div>
        ...
        {reel.caption && (
          <div className="mb-2">
            <p
              className={cn('text-white/90 text-[13px] leading-relaxed', !expandCaption && 'line-clamp-2')}
        ...
        {reel.hashtags?.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-2.5">
            {reel.hashtags.slice(0, 4).map((tag, i) => (
              <span key={i} className="text-[11px] font-medium text-primary/90 bg-white/10 px-2 py-0.5 rounded-full backdrop-blur-sm">
                #{tag}
              </span>
            ))}
          </div>
      ...
      <div className="absolute right-2.5 bottom-28 z-20 flex flex-col items-center gap-4">
        {/* Like */}
        ...
        {/* Share */}
        <button onClick={onShare} className="flex flex-col items-center gap-0.5 active:scale-110 transition-transform">
          <div className="w-11 h-11 rounded-full bg-black/20 backdrop-blur-sm flex items-center justify-center">
