  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <DashboardV2Header
        ...
        alerts={[
          ...(recentShipments.filter(s => s.status === 'new').length > 0 ? [{ id: 'pending-gen', message: `⏳ ${recentShipments.filter(s => s.status === 'new').length} شحنة معلقة تحتاج موافقة وتعيين ناقل`, severity: (recentShipments.filter(s => s.status === 'new').length > 5 ? 'critical' : 'warning') as const, icon: Clock }] : []),
