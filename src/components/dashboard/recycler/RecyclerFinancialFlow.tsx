...
const RecyclerFinancialFlow = () => {
  const { organization } = useAuth();

  const { data: ledger, isLoading } = useQuery({
    queryKey: ['recycler-financial', organization?.id],
    queryFn: async () => {
  ...
  const analysis = useMemo(() => {
    ...
    const monthlyMap = new Map<string, { received: number; paid: number }>();
    ledger.forEach(e => {
      const month = e.entry_date?.substring(0, 7) || '';
      const existing = monthlyMap.get(month) || { received: 0, paid: 0 };
      if (e.entry_type === 'credit') existing.received += Math.abs(e.amount);
      else existing.paid += Math.abs(e.amount);
    ...
    const chartData = Array.from(monthlyMap.entries())
      ...
      .map(([m, d]) => ({
        ...
        'إيرادات معالجة': Math.round(d.received),
        'تكاليف تشغيل': Math.round(d.paid),
      }));
      ...
      <CardHeader>
        <div className="text-right">
          <CardTitle className="flex items-center gap-2 justify-end">
            ...
            التدفق المالي للمدوّر
          </CardTitle>
          <CardDescription>إيرادات رسوم المعالجة + مبيعات المواد المعاد تدويرها</CardDescription>
        </div>
      </CardHeader>
        ...
        {analysis.chartData.length > 0 && (
          <ResponsiveContainer width="100%" height={200}>
            <BarChart data={analysis.chartData}>
              ...
              <Tooltip />
              <Legend />
              <Bar dataKey="إيرادات معالجة" fill="hsl(142 71% 45%)" radius={[4, 4, 0, 0]} />
...
export default RecyclerFinancialFlow;
