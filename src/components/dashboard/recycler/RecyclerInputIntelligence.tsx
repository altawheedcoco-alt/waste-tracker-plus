...
const RecyclerInputIntelligence = () => {
  const { organization } = useAuth();

  const { data: shipments, isLoading } = useQuery({
    queryKey: ['recycler-input-intel', organization?.id],
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from('shipments')
        .select('id, waste_type, quantity, created_at, status')
      ...
      <CardHeader>
        <div className="text-right">
          <CardTitle className="flex items-center gap-2 justify-end">
            ...
            تحليل المواد الواردة
          </CardTitle>
          <CardDescription>مقارنة سنوية للكميات المستلمة (طن)</CardDescription>
        </div>
      </CardHeader>
      <CardContent>
...
export default RecyclerInputIntelligence;
