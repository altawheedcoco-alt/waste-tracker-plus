...
import { Printer, FileText, Receipt, X, Loader2 } from 'lucide-react';
...
const DashboardPrintReports = memo(() => {
  ...
  const { data: todayData } = useQuery({
    ...
    queryFn: async () => {
      ...
      const shipmentsRes: any = await supabase
        .from('shipments')
        .select('id, shipment_number, status, waste_type, weighbridge_net_weight, created_at, pickup_address, delivery_address, manual_vehicle_plate')
        .or(`generator_id.eq.${orgId},transporter_id.eq.${orgId},recycler_id.eq.${orgId},disposal_facility_id.eq.${orgId}`)
        .gte('created_at', todayStart.toISOString())
        .lte('created_at', todayEnd.toISOString())
        .order('created_at', { ascending: false });

      const ledgerRes: any = await supabase
        .from('accounting_ledger')
        .select('id, entry_type, entry_category, amount, description, entry_date')
      ...
      const notifRes: any = await supabase
        .from('notifications')
      ...
      const totalWeight = shipments.reduce((s, sh) => s + (sh.weighbridge_net_weight || 0), 0);
      const totalRevenue = ledger.filter(l => l.entry_type === 'credit').reduce((s, l) => s + l.amount, 0);
      const totalExpenses = ledger.filter(l => l.entry_type === 'debit').reduce((s, l) => s + l.amount, 0);

      return { shipments, ledger, notifications, totalWeight, totalRevenue, totalExpenses };
    },
  });

  const statusLabel = (s: string) => {
    const map: Record<string, string> = {
      pending: 'معلقة', in_transit: 'في الطريق', delivered: 'تم التسليم',
      confirmed: 'مؤكدة', cancelled: 'ملغاة', completed: 'مكتملة',
    };
    return map[s] || s;
  };

  const printReport = useCallback((type: 'receipt' | 'a4') => {
    ...
    const { shipments, ledger, totalWeight, totalRevenue, totalExpenses } = todayData;
...
${shipments.map((s, i) => `<tr><td>${i + 1}</td><td>${s.shipment_number || '-'}</td><td>${statusLabel(s.status || '')}</td><td>${s.weighbridge_net_weight || 0}</td></tr>`).join('')}
</table>
<div class="line"></div>
` : ''}
...
${ledger.map(l => `<tr><td>${l.entry_type === 'credit' ? '⬆' : '⬇'}</td><td>${(l.description || l.entry_category || '').substring(0, 20)}</td><td>${l.amount.toLocaleString('ar-EG')}</td></tr>`).join('')}
</table>
<div class="line"></div>
` : ''}
...
<tbody>
${shipments.map((s, i) => `<tr>
  <td>${i + 1}</td>
  <td>${s.shipment_number || '-'}</td>
  <td>${s.pickup_address || '-'}</td>
  <td>${s.delivery_address || '-'}</td>
  <td>${s.waste_type || '-'}</td>
  <td>${s.weighbridge_net_weight || 0} كجم</td>
  <td>${s.manual_vehicle_plate || '-'}</td>
  <td><span class="badge ${s.status === 'delivered' ? 'badge-success' : s.status === 'in_transit' ? 'badge-info' : 'badge-warning'}">${statusLabel(s.status || '')}</span></td>
...
<tbody>
${todayData.notifications.map((n, i) => `<tr>
  <td>${i + 1}</td>
  <td>${n.title || '-'}</td>
...
<div class="footer">
  ...
  <p>هذا التقرير آلي ولا يحتاج إلى توقيع</p>
    ...
    const printWindow = window.open('', '_blank', type === 'receipt' ? 'width=320,height=600' : 'width=800,height=1000');
    if (printWindow) {
      ...
      <DropdownMenuContent align="end" className="w-56">
        <DropdownMenuItem onClick={() => printReport('receipt')} className="gap-2 cursor-pointer">
          ...
          <div>
            ...
            <div className="text-xs text-muted-foreground">تقرير سريع لطابعة الإيصالات</div>
          </div>
        </DropdownMenuItem>
        <DropdownMenuSeparator />
        <DropdownMenuItem onClick={() => printReport('a4')} className="gap-2 cursor-pointer">
          ...
          <div>
            <div className="font-medium">تقرير A4 مفصل</div>
            <div className="text-xs text-muted-foreground">تقرير كامل مع جداول وإحصائيات</div>
          </div>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
