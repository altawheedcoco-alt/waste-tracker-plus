...
export const STATUS_CONFIG: Record<string, { color: string; label: string; icon: React.ReactNode }> = {
  ...
  approved: { color: 'bg-primary/15 text-primary', label: 'معتمدة', icon: createElement(CheckCircle, { className: 'h-4 w-4' }) },
  in_transit: { color: 'bg-secondary text-secondary-foreground', label: 'في الطريق', icon: createElement(Truck, { className: 'h-4 w-4' }) },
  delivered: { color: 'bg-accent text-accent-foreground', label: 'تم التسليم', icon: createElement(CheckCircle, { className: 'h-4 w-4' }) },
...
export const HAZARD_LEVELS: Record<string, { label: string; color: string }> = {
  ...
  medium: { label: 'متوسط', color: 'bg-secondary text-secondary-foreground' },
