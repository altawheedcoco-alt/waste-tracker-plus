...
import { PenTool, ShieldCheck, Clock, Stamp, Eye } from 'lucide-react';
import { supabase } from '@/integrations/supabase/client';
import { format } from 'date-fns';
import { ar } from 'date-fns/locale';

const UnifiedShipmentPrint = lazy(() => import('@/components/shipments/unified-print/UnifiedShipmentPrint'));
...
const ShipmentSignaturesCard = ({ shipmentId }: ShipmentSignaturesCardProps) => {
  ...
  return (
    <>
      <Card>
        <CardHeader className="py-3 px-4">
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            <button
              ...
              className="mr-auto flex items-center gap-1 text-[10px] text-primary hover:underline"
            >
        ...
        <CardContent className="px-4 pb-3 space-y-3">
          {signatures.map((sig, idx) => (
            <div
              key={sig.id}
              className="cursor-pointer hover:bg-muted/50 rounded-lg transition-colors -mx-1 px-1"
              onClick={() => setShowDocPreview(true)}
            >
              ...
              <div className="flex items-start gap-3">
                ...
                <Eye className="w-4 h-4 text-muted-foreground/50 mt-1 flex-shrink-0" />
              </div>
            </div>
          ))}
        </CardContent>
      ...
      {/* Document preview showing signatures in place */}
