...
import { getFieldsForOrgType, type FieldConfig } from './orgTypeFieldsConfig';
...
const groupLabels: Record<string, string> = {
  org_info: 'بيانات الجهة',
  legal: 'البيانات القانونية',
  representative: 'بيانات المسؤول',
...
export const CompanyRegistrationForm = ({ onSubmit, onBack, defaultOrgType }: CompanyRegistrationFormProps) => {
  ...
  // الحقول الديناميكية حسب نوع الجهة
  const dynamicFields = useMemo(() => {
    return getFieldsForOrgType(formData.organizationType);
  ...
  // Inline validation on blur — يستخدم الحقول الديناميكية
  const handleBlur = (field: keyof CompanyFormData) => {
    const value = formData[field]?.trim();
    const newErrors = { ...errors };
    
    const fieldConfig = dynamicFields.find(f => f.key === field);
    
    if (fieldConfig?.required && !value) {
      newErrors[field] = `يرجى إدخال ${fieldConfig.label}`;
  ...
  const validate = (): boolean => {
    ...
    // تحقق إضافي من الحقول الإلزامية الديناميكية
    for (const field of dynamicFields) {
      if (field.required && !trimmedData[field.key as keyof CompanyFormData]?.trim()) {
        if (!newErrors[field.key]) {
          newErrors[field.key] = `يرجى إدخال ${field.label}`;
        }
      }
    }
  ...
  /** عرض حقل واحد ديناميكياً */
  const renderField = (config: FieldConfig) => {
    const key = config.key as keyof CompanyFormData;
    const value = formData[key] || '';

    if (config.type === 'textarea') {
      return (
        <div key={config.key} className={`space-y-1 ${config.fullWidth ? 'col-span-2' : ''}`}>
    ...
    return (
      <div key={config.key} className={`space-y-1 ${config.fullWidth ? 'col-span-2' : ''}`}>
        ...
        <Input
          ...
          className={`h-8 text-xs ${errors[config.key] ? 'border-destructive' : ''}`}
          dir={config.dir}
  ...
  // الوصف الديناميكي حسب نوع الجهة
  const orgTypeLabel = organizationTypes.find(t => t.value === formData.organizationType)?.label || 'شركة';
    ...
    <div className="space-y-4 max-h-[70vh] overflow-y-auto px-1">
      ...
      <div className="text-center border-b pb-3">
        <h2 className="text-base font-bold flex items-center justify-center gap-2">
          ...
          تسجيل {orgTypeLabel} جديدة
        </h2>
        <p className="text-xs text-muted-foreground mt-1">
          إضافة {orgTypeLabel} مع البيانات القانونية الكاملة
        </p>
      </div>
      ...
      {/* نوع الشركة — دائماً أول حقل */}
      <div className="space-y-1">
        <Label className="text-xs">نوع الجهة <span className="text-destructive">*</span></Label>
        <Select
      ...
      {/* الحقول الديناميكية — تظهر بعد اختيار النوع */}
      {formData.organizationType && (
        <>
          {/* عرض كل مجموعة */}
          {(['org_info', 'legal', 'representative', 'activity'] as const).map(groupKey => {
            const fields = groupedFields[groupKey];
            if (!fields || fields.length === 0) return null;
            return (
              <div key={groupKey} className="space-y-2">
                <p className="text-[11px] font-semibold text-muted-foreground border-b border-border/40 pb-1">
          ...
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2 space-y-1">
                <Label className="text-xs font-semibold">طريقة التسجيل <span className="text-destructive">*</span></Label>
              ...
              {regMethod === 'email' ? (
                <div className="space-y-1">
                  <Label className="text-xs">البريد الإلكتروني للدخول <span className="text-destructive">*</span></Label>
                  <Input
                    type="email"
                ...
                <div className="space-y-1">
                  <Label className="text-xs">رقم الهاتف للدخول <span className="text-destructive">*</span></Label>
                  <Input
      ...
      <div className="flex gap-2 pt-2 border-t">
        ...
        <Button
          ...
          disabled={loading || !termsAccepted || !formData.organizationType}
          size="sm"
          className="flex-1"
        >
          <Building2 className="ml-1 w-3 h-3" />
          {loading ? 'جاري الحفظ...' : `تسجيل ${orgTypeLabel}`}
        </Button>
      </div>
    </div>
  );
