...
// Industrial search types for Google Places
...
serve(async (req) => {
  ...
  try {
    ...
    const { source = "google" } = await req.json().catch(() => ({}));

    console.log(`Fetching industrial facilities from ${source}...`);
    ...
    const localData = [
      ...
      { name: 'مصنع التوحيد للأخشاب - السادس من أكتوبر', name_ar: 'مصنع التوحيد للأخشاب - السادس من أكتوبر', facility_type: 'factory', city: 'السادس من أكتوبر', governorate: 'الجيزة', latitude: 29.9450, longitude: 30.9320, osm_id: 'local-tawhid-wood', is_verified: true },
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `تم جلب وتخزين ${inserted} منشأة صناعية من ${source === "google" ? "Google Maps" : "OpenStreetMap"}`,
        stats: {
          source: source,
