...
serve(async (req) => {
  ...
  try {
    ...
    // Get auth token to fetch user-specific data
...
- أضف **التاريخ** والمرجع في بداية كل مستند: مثل "التاريخ: ${new Date().toLocaleDateString('ar-EG')}"
- استخدم **العناوين الفرعية** (## و ###) لتقسيم المحتوى بوضوح
- استخدم **جداول Markdown** لكل البيانات الرقمية والمقارنات
...
- اختم المستندات بـ **قسم التوقيعات** عند الحاجة (مثل: "التوقيع: _______________")
- اترك فراغات واضحة بين الأقسام
- كن دقيقاً في الأرقام والتواريخ
- استخدم البيانات الحقيقية المتوفرة من قاعدة البيانات
- عند إنشاء خطط، قسمها بوضوح (سنوي → شهري → أسبوعي → يومي) في جداول منفصلة
