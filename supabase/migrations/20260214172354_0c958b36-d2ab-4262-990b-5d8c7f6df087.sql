...
-- Make declared_by_user_id and declaration_text nullable for auto-generated declarations
ALTER TABLE public.delivery_declarations ALTER COLUMN declared_by_user_id DROP NOT NULL;
ALTER TABLE public.delivery_declarations ALTER COLUMN declaration_text SET DEFAULT 'إقرار تلقائي';
ALTER TABLE public.delivery_declarations ALTER COLUMN declaration_text DROP NOT NULL;

-- Update the function to include declaration_text
...
BEGIN
  ...
  IF NEW.status = 'approved' AND OLD.status != 'approved' THEN
    v_decl_text := 'أقر أنا الممثل المفوض عن ' || COALESCE(v_generator_name, 'الجهة المولدة') || ' بتسليم شحنة النفايات رقم ' || v_shipment_number || ' بكمية ' || v_quantity || ' ' || v_unit;
  ...
  IF NEW.status IN ('in_transit', 'collecting') AND OLD.status NOT IN ('in_transit', 'collecting') THEN
    v_decl_text := 'أقر أنا الممثل المفوض عن ' || COALESCE(v_transporter_name, 'الجهة الناقلة') || ' باستلام ونقل شحنة النفايات رقم ' || v_shipment_number || ' بكمية ' || v_quantity || ' ' || v_unit;
    INSERT INTO public.delivery_declarations (
      shipment_id, shipment_number, declaration_type, declared_by_organization_id,
      generator_name, transporter_name, waste_type, quantity, unit, status, auto_generated, declared_at, declaration_text
    ) VALUES (
  ...
  -- On delivered or confirmed: recycler declaration + receipt
  IF NEW.status IN ('delivered', 'confirmed') AND OLD.status NOT IN ('delivered', 'confirmed') THEN
    v_decl_text := 'أقر أنا الممثل المفوض عن ' || COALESCE(v_recycler_name, 'جهة التدوير') || ' باستلام شحنة النفايات رقم ' || v_shipment_number || ' بكمية ' || v_quantity || ' ' || v_unit;
    INSERT INTO public.delivery_declarations (
      shipment_id, shipment_number, declaration_type, declared_by_organization_id,
      generator_name, transporter_name, waste_type, quantity, unit, status, auto_generated, declared_at, declaration_text
    ) VALUES (
