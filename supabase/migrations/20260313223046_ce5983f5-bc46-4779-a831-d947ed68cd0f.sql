...
BEGIN
  ...
  SELECT o.name INTO _gen_name FROM organizations o WHERE o.id = NEW.generator_id;
  ...
  IF NEW.transporter_id IS NOT NULL THEN
    ...
    SELECT p.user_id, '📦 شحنة جديدة #' || _ship_num, 
      'تم إسناد شحنة جديدة إليكم من ' || _gen_name,
      'new_shipment', false, NEW.id
    FROM profiles p WHERE p.organization_id = NEW.transporter_id AND p.is_active = true AND p.user_id IS NOT NULL;
  END IF;

  IF NEW.recycler_id IS NOT NULL AND NEW.recycler_id IS DISTINCT FROM NEW.transporter_id THEN
    INSERT INTO notifications (user_id, title, message, type, is_read, shipment_id)
    SELECT p.user_id, '📦 شحنة واردة #' || _ship_num, 
      'شحنة جديدة في الطريق إليكم من ' || _gen_name,
      'new_shipment', false, NEW.id
    FROM profiles p WHERE p.organization_id = NEW.recycler_id AND p.is_active = true AND p.user_id IS NOT NULL;
  END IF;
