...
BEGIN
  ...
  IF NEW.status = 'approved' AND OLD.status != 'approved' THEN
    v_declaring_org := COALESCE(NEW.generator_id, NEW.transporter_id);
    IF v_declaring_org IS NOT NULL THEN
      ...
      INSERT INTO public.delivery_declarations (
        ...
        NEW.id, v_shipment_number, 'generator_delivery', v_declaring_org,
  ...
  IF NEW.status IN ('in_transit', 'collecting') AND OLD.status NOT IN ('in_transit', 'collecting') THEN
    v_declaring_org := COALESCE(NEW.transporter_id, NEW.generator_id);
    IF v_declaring_org IS NOT NULL THEN
      ...
      INSERT INTO public.delivery_declarations (
        ...
        NEW.id, v_shipment_number, 'transporter_delivery', v_declaring_org,
        v_generator_name, v_transporter_name, v_waste_type, v_quantity, v_unit, 'confirmed', true, now(), v_decl_text
      ) ON CONFLICT DO NOTHING;
    END IF;
  ...
  IF NEW.status IN ('delivered', 'confirmed') AND OLD.status NOT IN ('delivered', 'confirmed') THEN
    v_declaring_org := COALESCE(NEW.recycler_id, NEW.transporter_id);
    IF v_declaring_org IS NOT NULL THEN
      ...
      INSERT INTO public.delivery_declarations (
        ...
        NEW.id, v_shipment_number, 'recycler_receipt', v_declaring_org,
        v_generator_name, v_transporter_name, v_waste_type, v_quantity, v_unit, 'confirmed', true, now(), v_decl_text
      ) ON CONFLICT DO NOTHING;
    END IF;
